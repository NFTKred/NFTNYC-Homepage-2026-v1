...
function buildOutreachDraft(speaker: Speaker, resource: Resource | undefined): OutreachDraft {
  ...
  // Image URL: dynamically rendered "Latest on X" section preview with this
  // resource as the top card. Generated by the /api/card-image/<id> Vercel
  // edge function on demand — no storage, no external screenshot services,
  // no quotas, always reflects current data.
  //
  // The ?v=<timestamp> cache-buster is critical: Gmail and other email
  // clients aggressively cache images by URL via their own image proxies
  // (googleusercontent.com etc.). Without a unique URL per draft, an
  // edited resource would keep showing the old image in fresh emails for
  // days. Generating the buster here ensures every Copy Draft click loads
  // an uncached image.
  const imageUrl = resource
    ? `${window.location.origin}/api/card-image/${resource.id}?v=${Date.now()}`
    : null;
  ...
  // The card image links to the vertical page (since it shows the resource
  // featured at the top of that page).
  const imageLinkHref = pageUrl;
                      ...
                      <div style={{ display: 'flex', gap: '0.25rem', alignItems: 'center' }}>
                        ...
                        <button
                          onClick={async () => {
                            ...
                            const cardUrl = `${window.location.origin}/api/card-image/${relatedResource.id}?v=${Date.now()}`;
                            try {
