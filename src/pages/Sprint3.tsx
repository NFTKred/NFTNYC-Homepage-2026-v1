      ...
      <div className="vibesprint sprint3">
        <div className="wrap" style={{ paddingTop: 96 }}>
          ...
          <section id="register">
            ...
            <p className="lead">One question first, because it decides how much we need from you.</p>

            <div className="regpick">
              <details className="reg" name="reg">
                <summary>Yes, I registered for Sprint 1 or Sprint 2</summary>
                <div className="regbody">
                  <p>
                    Your registration still covers your entry, your kit, and your .Kred domain
                    claim, so there is nothing to supply again. One line is all we need, and it
                    tells us how many builders to expect and how to staff the support sessions.
                  </p>
                  {!quickSent && (
                    <form className="quickform" onSubmit={onSubmitQuick} noValidate>
                      <input
                        id="qe"
                        type="email"
                        required
                        placeholder="The email you registered with"
                        aria-label="The email you registered with"
                        value={quickEmail}
                        onChange={(e) => { setQuickEmail(e.target.value); setQuickError(null); }}
                      />
                      <button className="btn" type="submit" disabled={quickSending}>
                        {quickSending ? "Saving…" : "I am in for Sprint 3"}
                      </button>
                    </form>
                  )}
                  {quickError && (
                  ...
                  <p className="quicknote">
                    That is the whole thing. No terms to accept again, no platform to pick, no
                    details to repeat.
                  </p>
                </div>
              </details>
                ...
                <div className="regbody">
                  ...
                  {!submitted && (
                    <form onSubmit={onSubmit} noValidate>
                      ...
                      <div className="field">
                        <label htmlFor="rName">Name or agent name</label>
                        <input
                          id="rName"
                          name="name"
                          type="text"
                      ...
                      <div className="field">
                        <label htmlFor="rSegment">Which approach is yours</label>
                        <select
                          id="rSegment"
                          value={segment}
                          onChange={(e) => setSegment(e.target.value)}
                        >
                          <option>NFT.NYC creator, design what an argument looks like</option>
                          <option>Hugging Face builder, make an agent hold a position</option>
                          <option>Both</option>
                          <option>Not sure yet</option>
                        </select>
                      </div>
                      <div className="field">
                        <label htmlFor="rTool">Build platform</label>
                        <select
                          id="rTool"
                          value={buildTool}
                        ...
                        >
                          ...
                          <option>Hugging Face Spaces</option>
                          <option>Grok Bot or another persistent agent platform</option>
                          <option>Replit</option>
                          <option>Vercel and GitHub</option>
                      ...
                      <div
                        ...
                        style={{
                          background: "rgba(241, 86, 33, 0.08)",
                          border: "1px solid rgba(241, 86, 33, 0.45)",
                          borderRadius: 8,
                          padding: "14px 16px",
                        }}
                      >
                        ...
                        <p className="form-note" style={{ marginTop: 6 }}>
                          ...
                          register — it becomes your actual domain, so choose one you want to keep.
                        </p>
                      </div>

                      <div className="field full">
                        <label htmlFor="rDomain">Claim your free Kred domain</label>
                        <input
                          id="rDomain"
                          name="domain"
                          required
                          ...
                          value={domain}
                          onChange={(e) => { setDomain(e.target.value); setFormError(null); }}
                        />
                        <span className="form-note">
                          {domainCheck.state === "checking" && "Checking availability…"}
                          {domainCheck.state === "available" && (
                            <span style={{ color: "var(--s3-green)" }}>
                              {domain.trim().toLowerCase().replace(/\.kred$/i, "")}.Kred is available.
                            </span>
                          )}
                          {domainCheck.state === "taken" && (
                            <span style={{ color: "#F15621" }}>
                              That name is taken{domainCheck.message ? `: ${domainCheck.message}` : " — try another."}
                            </span>
                          )}
                          {(domainCheck.state === "idle" || domainCheck.state === "unknown") &&
                            (domainCheck.message ?? "Letters, numbers and hyphens. Free with your kit.")}
                        </span>
                      </div>
                      ...
                      <div className="agree">
                        ...
                        <label
                          htmlFor="rAgree"
                          style={{ fontSize: 13, letterSpacing: 0, textTransform: "none", color: "var(--color-text-muted)" }}
                        >
                          I accept the{" "}
                          <a
                          ...
                          , and I understand XP carries a 60-day expiry with no money value.
                        </label>
                      </div>

                      <div className="form-actions">
                        <button className="btn" type="submit" disabled={sending || !agree}>
                          {sending ? "Registering…" : "Register and count me in"}
                        </button>
                        <span className="cta-note">Free. One registration covers every sprint.</span>
                      </div>
                      {formError && (
                        <p className="form-note" role="alert" style={{ gridColumn: "1 / -1", color: "#F15621" }}>
                          {formError}
                        </p>
                      )}
                    </form>
